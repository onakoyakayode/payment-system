  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      <header>
        ...
        <p className="text-slate-500">Create interactive assessments manually or with AI assistance.</p>
      </header>
      ...
      <div className="bg-indigo-600 rounded-3xl p-8 text-white shadow-xl shadow-indigo-100 flex flex-col md:flex-row items-center gap-8">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-bold uppercase tracking-widest text-indigo-200">AI Powered Creator</span>
          </div>
          <h2 className="text-2xl font-bold mb-4">Generate questions in seconds</h2>
          <p className="text-indigo-100 mb-6 text-sm opacity-90 leading-relaxed">
            Enter a subject, topic, or even paste a lesson transcript to automatically generate high-quality multiple choice questions.
          </p>
              ...
              <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pl-11">
                  {q.options.map((opt, optIdx) => (
                    ...
                    >
                      {opt}
                      {optIdx === q.correctAnswer && <span className="ml-2 text-[10px] font-bold uppercase bg-emerald-100 px-1.5 py-0.5 rounded">Correct</span>}
                    </div>
                  ))}
                </div>
              </div>
