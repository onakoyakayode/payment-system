...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (view) {
      ...
      case 'QUIZ_MAKER': return <QuizMaker />;
      ...
      default: return (
        <div className="flex flex-col items-center justify-center h-[60vh] text-center p-12 bg-white rounded-3xl border border-dashed border-slate-200">
          ...
          <p className="text-slate-500 max-w-sm mb-8">The {view.toLowerCase()} module is being prepared for the 2024/2025 academic session.</p>
          <button 
  ...
  return (
    <RootLayout view={view} setView={setView} user={user} onLogout={handleLogout}>
      {renderContent()}
    </RootLayout>
  );
};
